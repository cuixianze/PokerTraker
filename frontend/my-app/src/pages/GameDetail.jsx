...
function GameDetail() {
  ...
  const rake = gameDetail?.rake ?? 0; // Default to 0 if undefined
  const shark = gameDetail?.shark ?? {}; // Default to an empty object
  const fish = gameDetail?.fish ?? {}; // Default to an empty object
  const players = gameDetail?.players ?? []; // Default to an empty array

  return (
    <div className="common">
      <Navbar />
      <div className="gameDetail_container">
        <div className="gameDetail_box">
          <div className="gameDetail">
            <div className="gd_title">GameDetail</div>
            <p className="gd_date">
              <strong>Date:</strong> 날짜 정보 없음
            </p>
          </div>
          <div className="gameDetail_content">
            <div className="gd_tableBox">
              <table className="gd_table">
                <thead>
                  <tr>
                    <th>Username</th>
                    <th>Profit/Loss</th>
                ...
                <tbody>
                  {players.map((player, index) => (
                    <tr key={index}>
                      <td className="gd_td">{player.username ?? "Unknown"}</td>
                      <td
                        className={
                          player.profitLoss >= 0 ? "positive" : "negative"
                        }
                      >
                        {player.profitLoss ?? 0}
                      </td>
            ...
            <div className="gd_rakeBox">
              <div className="gd_rake">
                <p>
                  <strong>Total Rake:</strong> {rake}
                </p>
              </div>
              <div className="gd_sharkfish">
                <div className="gd_shark">
                  <svg
                  ...
                  <p>
                    <strong>Shark:</strong> {shark.username ?? "Unknown"} (Total
                    PnL: {shark.totalPnL ?? 0})
                  </p>
                </div>
                <div className="gd_fish">
                  <svg
                    xmlns="https://www.w3.org/2000/svg"
                    width="200px"
                    height="200px"
                  ...
                  <p>
                    <strong>Fish:</strong> {fish.username ?? "Unknown"} (Total
                    PnL: {fish.totalPnL ?? 0})
                  </p>
                </div>
              </div>
