...
function GameCreationForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:8080/create/gameWithPlayers', gameData);
        ...
        {players.map((player, index) => (
          <div key={index} className="player-input">
            ...
            <input
              ...
              onChange={(e) => handlePlayerChange(index, 'profitLoss', e.target.value)} // No conversion until submit
              required
            />
          </div>
        ))}
